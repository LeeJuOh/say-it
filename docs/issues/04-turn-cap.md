...
Hook 레벨 턴캡 강제. **S1(분출) 단계에만 적용** — S2-S4는 구조화된 단계라 캡 불필요.
...
Hook이 session_state에서 현재 단계 + 턴수 읽음 → S1이고 캡 조건 충족 시 → stdout으로 `CAP_TRIGGERED: SOFT` 또는 `CAP_TRIGGERED: HARD` 포함 system-reminder 주입 → 모델이 초대 문구로 물음(SOFT) 또는 강제 전환(HARD).
